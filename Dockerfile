...
RUN apt-get update && apt-get install -y \
    && apt-get install -y build-essential && apt-get install -y gcc && apt-get install -y git && apt-get install -y cmake && apt-get install -y wget

RUN wget https://github.com/aquynh/capstone/archive/4.0.2.tar.gz && tar xzf 4.0.2.tar.gz && cd capstone-4.0.2 && ./make.sh install  && cd / 

RUN apt-get install -y python3 && apt-get install -y python3-dev && apt-get install -y python-setuptools && apt-get install -y libz3-dev && apt-get install -y libboost-all-dev
